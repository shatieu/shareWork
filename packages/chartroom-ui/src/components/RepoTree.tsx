...
export interface RepoTreeProps {
  ...
  /** Opens the host-owned folder-picker registration modal (RegisterRepoModal). */
  onOpenRegister: () => void;
}
...
/**
 * LOCAL REPOS panel (design 2a's left column): repo rows with brass initial avatars, alert badges,
 * collapsible folder groups built from real doc paths, active-doc rust highlight, a register-repo
 * explainer popover, and a per-repo "open Claude session" hover action. Collapses to a 52px brass
 * icon rail. Expansion state persists in localStorage (repos via the host, folders locally).
 */
export function RepoTree({
  ...
  if (collapsed) {
    return (
      <nav className="panel repo-rail" aria-label="Local repos">
        <button type="button" className="repo-rail__expand" onClick={() => onSetCollapsed(false)} title="expand" aria-label="Expand repo panel">
          »
        </button>
  ...
  function renderFolder(repo: RepoSummary, folder: FolderNode, depth: number): ReactElement {
    ...
    return (
      <li key={folderKey}>
        ...
        {isOpen && <ul>{renderChildren(repo, folder, depth + 1)}</ul>}
      </li>
    );
  }
  ...
  return (
    <nav className="panel repo-tree" aria-label="Local repos">
            ...
            return (
              <li key={repo.id}>
                <div className="tree-repo-row">
                  ...
                  {alert > 0 && <span className="badge-alert">{alert}</span>}
                ...
                {isOpen &&
                  (docs === undefined ? (
                    ...
                    tree && <ul>{renderChildren(repo, tree, 0)}</ul>
                  ))}
              </li>
            );
          })}
      ...
      <div className="repo-tree__footer">
        <button type="button" className="repo-tree__register" onClick={onOpenRegister}>
          ＋ register repo…
        </button>
      </div>
    </nav>
