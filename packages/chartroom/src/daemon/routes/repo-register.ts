 ...
 * Injected the same way as the claude-session spawner so route tests never touch the real home
...
/**
 * `POST /api/repos/register` `{ path }` — live counterpart of the `chartroom register` CLI for the
 * UI's folder picker. Unlike the CLI-era design (static mounts fixed at boot), raw assets are now
